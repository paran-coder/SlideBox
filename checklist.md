...
- [ ] **Verify(c): "PDF 열기" → 새 탭에서 PDF 표시 — 사용자가 크롬에서 안 열린다고 확인(2026-07-11), 원인 파악: `window.open()`이 파일을 다 읽은 뒤(await 여러 번 지난 뒤) 호출돼 크롬 팝업 차단에 걸림 → 클릭 즉시 빈 탭을 먼저 열고 나중에 주소만 바꾸는 방식으로 수정함. 재배포 후 실제로 열리는지 재확인 필요**
...
| 5 | "PDF 열기"로 새 탭에 열림, PPTX 경로 클릭 복사+안내 | ⚠️ 부분 확인 | PPTX 경로 복사는 확인됨. "PDF 열기"는 크롬 팝업 차단 버그 발견(2026-07-11) → 수정 완료, 재배포 후 최종 확인 필요 |
