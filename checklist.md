...
| 5 | "PDF 열기"로 새 탭에 열림, PPTX 경로 클릭 복사+안내 | ⚠️ 부분 확인 | PPTX 경로 복사는 사용자가 직접 클릭해 결함을 찾고 수정까지 확인됨. "PDF 열기"는 `originals/` 제거 이후 재확인 필요 |
| 6 | 제목·메모·태그 즉시 저장 + "저장됨" 안내 | ✅ 확인됨 | text123 태그 라운드에서 반복 확인 |
| 7 | 같은 file_key PDF+PPTX 자동으로 한 레퍼런스로 묶임 | ✅ 확인됨 | 일괄 가져오기 4/4 성공 |
| 8 | 일괄 가져오기 여러 개, 개별 진행 상태 표시, 실패가 전체 안 막음, 덮어쓰기 확인창 | ✅ 확인됨 | 4개 중 1개 50MB 차단 + 나머지 진행 확인, 재가져오기 시 덮어쓰기 확인창도 실제로 거침(그 과정에서 read/write 버그 발견·수정). "손상된 파일" 케이스만 미확인으로 별도 표시(Task 2c) |
