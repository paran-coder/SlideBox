...
- [x] **네이버 웨일 브라우저 크래시 — 여러 차례 재현·수정 끝에 해결.** 진짜 원인은 `readLibrary` 호출부의 처리되지 않은 `NotAllowedError`(권한이 세션 도중 만료되면 발생) 예외였다. 홈/상세/설정/일괄가져오기 네 곳을 try/catch로 감쌌고, 추가로 화면 안에서 바로 재연결(`showDirectoryPicker` 재호출)하는 코드를 전부 제거하고 재연결은 `/settings`에서만 하도록 통일했다. **사용자 재테스트로 크래시 해결 확인 완료(2026-07-05).**
- [x] Task 2, 4, 5의 보류된 수동 검증 항목 상당수를 실제 데이터로 재확인(일괄 가져오기 정상 동작 확인 등). 남은 것: AI 태깅 사전 위반 원인 미확정, 새 태그가 홈 필터에 반영되는지 재확인 필요
