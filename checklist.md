...
- [x] 설정 화면에 태그 관리(이름 변경/삭제/추가, 연쇄 정리) 기능 추가(사용자 요청)
- [x] 홈 화면 기본 보기(파일 보기)와 파일/슬라이드 보기별 페이지네이션(10/30/50/100, localStorage 유지) 추가
- [x] **네이버 웨일 브라우저 크래시 — 코드로 고칠 수 있는 부분은 다 고쳤으나, 최종 결론은 "웨일 자체의 버그".** `readLibrary`의 처리되지 않은 `NotAllowedError` 예외 수정, 화면 내 재연결 제거(재연결은 `/settings`에서만) 등은 실제 개선이었지만, 프로덕션 배포본(오류 오버레이가 없는 환경)에서도 크래시가 재현되어 원인이 웨일의 `showDirectoryPicker()` 네이티브 구현 버그임을 확인했다. 크롬 재현 안 됨/가정용 기기/DRM·VPN 없음까지 확인해 앱 코드 문제가 아님을 최종 확인. 하드 차단은 하지 않기로 결정(사용자 판단: 한국 유저의 웨일 사용률이 높음) — 비차단 경고 배너 유지. 자세한 경위는 context-notes.md 참조.
- [x] Task 2, 4, 5의 보류된 수동 검증 항목 상당수를 실제 데이터로 재확인(일괄 가져오기 정상 동작 확인 등). AI 태깅 "사전 위반" 제보는 태그 관리 목록(37개 정확히 일치) 확인 후 재현 안 됨으로 종결. 새 태그가 홈 필터에 반영되는지는 방어적 수정(포커스/뒤로가기 시 재조회) 후 재확인 필요
- [x] Vercel에 GitHub 저장소(https://github.com/paran-coder/SlideBox) 연결 및 배포 완료(환경변수 없음) — https://slidebox.vercel.app , 200 OK 확인
