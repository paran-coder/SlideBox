...
- **→ 결과: 사용자 재테스트로 확인됨(4개 중 4개 정상, 50MB 차단 1개 포함해 정상 동작).** "이상없는것 같아"로 확인 완료.

### 2026-07-05 추가 라운드: 실사용 중 발견된 버그 4건

같은 세션에서 사용자가 실제 데이터로 나머지 항목들을 테스트하며 한 번에 4가지를 보고했다.

1. **AI 태깅 결과에 사전에 없는 태그가 섞임.** `ai-tagging.ts`의 `filterKnown()`은 정확히 사전 이름 집합으로 필터링하고 있어 코드 리뷰만으로는 원인을 못 찾았다. **⚠️ 미해결 — 다음 세션에서 최우선 확인.** 사용자에게 구체적으로 어떤 태그 이름이 사전에 없는 것으로 보이는지 예시를 받아야 한다. 의심 지점: (a) `nameToId`가 이름→id의 flat Map이라 스타일/레이아웃/주제 간 이름이 중복되면 엉뚱한 kind의 id로 매핑될 수 있음(단, 이 경우도 "존재하는" 태그이지 "사전에 없는" 태그는 아님), (b) 실제로는 새로 만들어진 TagDef가 없는지(`library.tags` 배열 길이가 실행 전후로 늘었는지) 확인 필요.
2. **검색 후 상세로 갔다가 뒤로 가면 검색어가 사라짐.** `query`(및 `selectedTagIds`)가 컴포넌트 로컬 state였던 게 원인으로 추정. **수정:** `viewMode`와 같은 방식으로 URL 쿼리(`?q=`, `?tags=`)로 옮겼다. 검색어는 타이핑마다 URL을 갱신하면 버벅이므로, 로컬 state로 즉시 반영하고 300ms 멈췄을 때만 URL에 동기화하는 디바운스 방식을 썼다. 태그 필터는 클릭이 드문드문 일어나므로 즉시 URL에 반영한다. 공통 로직은 `updateSearchParams(patch)` 헬퍼로 통합.
3. **`TagEditor`의 "+ 태그" 팝업이 바깥을 클릭해도 안 닫힘.** 코드에 클릭 외부 감지 로직이 아예 없었다(명백한 누락). `containerRef` + `document`의 `mousedown` 리스너로 바깥 클릭 시 `setOpen(false)` 하도록 수정.
4. **상세 화면에서 만든 새 태그가 홈 화면 필터 목록에 안 보임.** `handleCreateFileTag` 등 상세 페이지의 쓰기 로직 자체는 정상으로 보인다(코드 리뷰상 `library.tags`에 제대로 추가되고 `writeLibrary`로 저장됨). 원인은 확정하지 못했지만, 홈 화면이 상세 페이지에서 돌아왔을 때 라이브러리를 다시 읽지 않고 예전 상태를 그대로 보여주는 것으로 추정된다(Next.js App Router의 클라이언트 라우터 캐시가 페이지 재마운트 없이 이전 렌더를 재사용하는 경우일 가능성). **수정(방어적):** 홈 화면의 라이브러리 로드 effect에 `focus`/`popstate`/`visibilitychange` 리스너를 추가해서, 탭이 다시 보이거나 포커스를 받거나 브라우저 뒤로/앞으로 가기가 발생할 때마다 라이브러리를 다시 읽어오도록 했다. **⚠️ 정확한 원인은 미확정 — 이 조치로 해결되는지 재확인 필요.** 만약 여전히 안 되면 Next.js 라우터 캐시 자체를 의심하고 `export const dynamic = 'force-dynamic'`이나 페이지에 `key={pathname}`을 주는 등 더 강한 조치를 고려해야 한다.
- **⚠️ 수동 검증 필요 — 다음 세션 최우선.** 이번 라운드의 2/3/4번 수정이 실제로 문제를 해결하는지, 그리고 1번(AI 태깅)의 구체적인 재현 사례를 받아 코드를 다시 봐야 한다.

### Task 6 배포 완료 (2026-07-05)

로컬 개발 서버가 자꾸 꺼지는 불편 때문에, 사용자 요청으로 이 시점에 배포를 먼저 진행하고 이후 수정은 배포된 URL에서 이어가기로 했다(원래 계획서 순서상으로는 수동 검증을 다 끝낸 뒤 배포하는 순서였지만, 실용적으로 순서를 바꿨다).

- GitHub 저장소: `https://github.com/paran-coder/SlideBox` (사용자가 미리 만들어둔 빈 저장소에 push). `main` 브랜치로 푸시.
- Vercel CLI(`paran-coder` 계정으로 이미 로그인되어 있었음)로 `vercel link` → GitHub 저장소 자동 연결됨 → `vercel --prod`로 배포.
- **프로젝트 이름 관련 주의:** 폴더명(`0705_SlideBox`)에 대문자가 있어 Vercel 프로젝트 이름 규칙(소문자만 허용)에 안 맞아서, `vercel link --project slidebox`로 명시적으로 지정해야 했다(Task 0에서 `create-next-app` 폴더명 문제와 같은 종류의 이슈).
- **배포 URL:** https://slidebox.vercel.app (프로덕션, 200 OK 확인). 환경변수는 설계대로 하나도 없음.
- `.vercel/`(로컬 프로젝트 링크 설정)을 `.gitignore`에 추가해 커밋에서 제외했다.
- **이제부터는 로컬 대신 배포 URL로 검증을 이어간다.** 코드를 고칠 때마다 `git push`만 하면 Vercel이 자동으로 다시 배포한다(GitHub 연결됨). 수동으로 `vercel --prod`를 실행할 필요는 없다.
- **남은 Task 6 항목:** 다른 컴퓨터/브라우저 프로필에서 전체 플로우 재검증(이제 배포 URL로 가능), 설계 문서 검증 기준 12개 확인, 이번 세션에서 발견된 미해결 버그(AI 태깅 사전 위반, 새 태그 홈 필터 반영) 확인.

### 웨일 크래시 — 최종 결론: 브라우저 자체 버그, 코드로 해결 불가 (2026-07-05)

배포(프로덕션) 사이트에서도 "설정에서 폴더 재연결" 흐름 중 크래시가 재현됐다. 이게 중요한 이유: **프로덕션 빌드에는 Next.js 개발 모드 오류 오버레이가 아예 없다** — 그런데도 크래시가 났으므로 "웨일이 오류 오버레이 렌더링을 못 견뎌서 죽는다"는 이전 가설은 폐기해야 한다. 즉 문제는 저희 JS 에러 처리가 아니라 **웨일의 `showDirectoryPicker()` 네이티브 구현 자체**에 있다.

- 웨일 공식 헬프센터(help.whale.naver.com)의 크래시 문제해결 가이드에 따라 사용자와 함께 원인을 좁혔다: (1) 크롬에서는 동일 동작이 크래시 안 남 → 원인이 앱 코드가 아님을 재확인, (2) 가정용 기기(회사/공공기관 보안 프로그램 문제 아님), (3) DRM/VPN/프록시 없음. 웨일 자체 버전은 4.38.386.14(공식 빌드, 64비트).
- **결론: 이건 네이버 웨일 브라우저 자체의 버그이고, 앱 코드로는 고칠 수 없다.** 사용자에게 웨일 크래시 시 생성되는 dmp 파일(`%LocalAppData%\Naver\Naver Whale\User Data\Crashpad\reports`)로 웨일 자체에 문제 신고를 하도록 안내했다.
- **하드 차단은 하지 않기로 결정했다** — 사용자 판단: "한국 유저들은 웨일을 많이 사용해. 차단하는건 선택사항에 없는 내용이야." 대신 현재 배포된 코드 상태(비차단 경고 배너: "이 브라우저(네이버 웨일)에서는 과거 폴더 접근 관련 문제가 있었습니다. 문제가 발생하면 Chrome 또는 Edge를 사용해 주세요")를 최종 상태로 유지한다.
- **다음 세션에서 웨일 관련 질문이 나오면:** 코드를 더 고치려 하지 말 것. 이미 할 수 있는 조치(readLibrary 예외 처리, 화면 내 재연결 제거, 경고 배너)는 다 했고, 남은 건 웨일 자체의 버그라 사용자가 네이버에 직접 제보해야 하는 영역이다.

### AI 태깅 "사전 위반" — 재현 안 됨, 오해였던 것으로 결론 (2026-07-05)

사용자가 "사전에 없는 태그도 섞여 있어"라고 보고했으나, `/settings`의 태그 관리 목록을 함께 확인한 결과 스타일 15/레이아웃 12/주제 10, 정확히 37개 프리셋 그대로였고 추가/이상 항목이 없었다. 구체적으로 어떤 태그가 문제였는지 다시 물어봤을 때 사용자가 "없는것 같은데?"로 답해, **실제 버그가 아니라 오해(다소 낯선 프리셋 이름을 못 알아본 경우 등)였던 것으로 결론짓는다.** `ai-tagging.ts`의 `filterKnown()` 로직 자체는 코드 리뷰로도 문제없어 보였으니, 추가 조치 없이 종결.

### 새 태그가 홈 필터에 안 보이던 문제 — 진짜 원인은 "설계대로 동작"이었음 (2026-07-05)

이전 라운드에서 "상세 화면에서 만든 새 태그가 홈 필터에 안 보인다"고 방어적으로 고쳤던 문제를 사용자가 재현 조건을 좁혀서 다시 확인했다. 결과: 새로 만든 태그(`text123`, 주제)는 **홈 화면 태그 필터 목록에는 정상적으로 나타났다.** 다만 그 태그를 **특정 슬라이드**에만 붙였는데, 홈 화면 "파일 보기" 모드에서 그 태그로 필터/검색하면 아무것도 안 걸렸다.

- **원인:** 버그가 아니라 계획서에 명시된 설계 그대로였다 — "파일 보기는 ref의 tag_ids 기준, 슬라이드 보기는 슬라이드의 tag_ids 기준"(Task 4 명세). 파일 보기는 원래 슬라이드별 태그를 보지 않는다. 사용자가 "슬라이드 보기"로 바꾸니 정상적으로 찾아졌다.
- **UX 개선 결정:** 사용자가 "그러면 파일보기와 슬라이드 보기에서 태그가 다르게 동작하는 게 헷갈린다"고 지적. 두 가지 안(① 파일 보기의 판정 기준을 슬라이드 태그까지 포함하도록 넓히기, ② 보기 모드별로 필터 칩 목록 자체를 다르게 보여주기) 중 **①번으로 결정**(더 직관적이고 구현이 단순함).
- **수정:** `page.tsx`의 `filteredRefs`(파일 보기용 필터링)에서 이제 `ref.tag_ids`뿐 아니라 `ref.slides.flatMap(s => s.tag_ids)`까지 합쳐서(`allTagIds`) 검색어 매칭과 태그 필터 매칭 양쪽에 사용한다. 즉 **파일 보기에서도 "이 파일 안의 슬라이드 중 하나라도 이 태그/검색어에 해당하면" 그 파일이 보인다.** 슬라이드 보기(`filteredSlideItems`)는 기존 그대로 슬라이드 단위로만 본다(변경 없음).
- 애초에 "새 태그가 홈 필터에 안 보인다"는 리포트 자체가, 이전 라운드에서 고친 focus/popstate 재조회 방어 코드 덕분인지 원래도 문제없었는지는 이번 재현으로는 확실히 가려지지 않았다(사용자가 이번엔 태그 목록 자체는 바로 보였다고 확인). 그 방어 코드는 부작용이 없으니 그대로 유지한다.
- **⚠️ 수동 검증 필요:** 파일 보기에서 슬라이드 전용 태그로 필터/검색했을 때 그 파일이 정상적으로 보이는지 재확인 필요.

### UI 폴리시 1차 (2026-07-11)

`/anthropic-skills:ui-polish` 스킬로 전체 화면(홈/가져오기/설정/상세)을 다시 점검했다. 이 앱은 마케팅 랜딩페이지가 없는 개인용 로컬 툴이라(설계 문서에 명시), "랜딩페이지 점검" 요청을 "첫 화면 포함 전체 화면 UI 완성도 점검"으로 재해석해서 진행했다고 사용자에게 설명하고 동의를 구한 뒤, 우선순위 1~6번을 순서대로 전부 적용했다.

1. **메타데이터/파비콘(버그 수정)** — `layout.tsx`의 `metadata`가 `create-next-app` 기본값("Create Next App" / "Generated by create next app")인 채로 방치되어 있었다. `title`(템플릿 포함)/`description`/`metadataBase`(`https://slidebox.vercel.app`)를 채우고, `lang="en"` → `lang="ko"`로 수정. 이전 세션에서 gptimage로 만들었다던 OG 이미지는 실제로는 `public/`에 존재하지 않았음(작업이 코드에 반영된 적이 없었음) — 별도 이미지 파일 대신 Next.js 파일 컨벤션(`app/icon.tsx`, `app/opengraph-image.tsx`, `next/og`의 `ImageResponse`)으로 코드 생성 방식을 택해 `src/app/icon.tsx`, `src/app/opengraph-image.tsx` 신설. `next build`에서 `/icon`, `/opengraph-image` 라우트로 정상 생성됨을 확인.
2. **로딩 상태 스켈레톤** — 홈/상세/가져오기 화면에서 `checkingDir`/`loadingLibrary` 대기 중 `return null`(빈 화면 깜빡임)이던 것을 `src/components/LoadingScreen.tsx`(신설, 검색바+카드 그리드 모양의 `animate-pulse` 스켈레톤)로 교체.
3. **버튼 색 위계 통일** — 지금까지 primary/secondary 버튼이 전부 `bg-black`으로 구분이 안 됐던 것을, accent 컬러(`indigo-600`/`hover:indigo-700`)를 도입해 모든 화면의 "주요 액션" 버튼(가져오기, PDF 열기, AI 태깅 실행, 저장, 폴더 선택, 파일 입력 버튼, 활성 탭/뷰토글, 선택된 태그 칩)에 일관 적용. destructive(라이브러리에서 제거)는 기존 red 유지, secondary는 neutral border 유지.
4. **카드 그림자** — `FileGrid`/`SlideGrid` 카드에 `shadow-sm hover:shadow-md transition-shadow` 추가(장식이 아니라 카드와 배경의 위계 구분 목적). 슬라이드 카드 확대 인터랙션(`group-hover:scale-[3]`)에는 `motion-reduce:transition-none` 추가(접근성).
5. **홈 상단 바 그룹핑** — 검색창+뷰토글(왼쪽 그룹)과 설정+가져오기(오른쪽 그룹)를 `justify-between`으로 분리해 스캔하기 쉽게 재배치. 기존엔 4개 요소가 구분 없이 한 줄에 나열돼 있었음.
6. **빈 라이브러리 상태 보강** — 텍스트 한 줄뿐이던 안내를 아이콘(인라인 SVG, 이미지 파일 없이) + 2줄 설명 + accent CTA 버튼으로 확장.

검증: `tsc --noEmit`/`eslint`/`next build` 전부 통과. Preview 브라우저로 `/settings` 화면 렌더링(accent 버튼, 레이아웃 정상), `document.title`이 "슬라이드박스"로 정상 표시되는 것 확인. File System Access API는 헤드리스 프리뷰 브라우저에서 실제 폴더 선택이 불가능해 홈 그리드/빈 상태/카드 그림자는 코드 리뷰 + 스타일 클래스 확인까지만 하고, 실제 브라우저에서의 최종 확인은 사용자 몫으로 남는다.

### 백로그 처리 완료 — 슬라이드 보기 호버 확대 설정 + 정렬 수정 (2026-07-11)

1. **슬라이드 보기 호버 확대 on/off 설정** — `src/lib/ui-prefs.ts` 신설(`getSlideHoverZoomEnabled`/`setSlideHoverZoomEnabled`, localStorage 키 `slidebox:slide-hover-zoom`, 기본값 켜짐). 설정 화면에 "화면 표시" 섹션과 체크박스 추가. `SlideGrid`가 마운트 시 이 값을 읽어(하이드레이션 불일치 방지 패턴 동일 적용) `group-hover:scale-[3]`/`group-hover:shadow-2xl`/`hover:z-20` 클래스 적용 여부를 결정하도록 `SlideCard`에 `hoverZoomEnabled` prop으로 전달.
2. **슬라이드 보기 정렬 수정** — CSS multi-column(`columns-2 sm:columns-3 md:columns-4`) masonry 방식을 걷어내고, `FileGrid`와 동일한 `grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4`로 교체. multi-column은 첫 번째 컬럼부터 위에서 아래로 채우는 방식이라 전체적으로 왼쪽에 쏠려 보였는데, grid는 행 단위로 좌→우 채워서 자연스럽게 정렬된다. 썸네일에 `aspect-video`를 명시해 그리드 셀 높이가 행마다 들쭉날쭉해지지 않게 함(FileGrid의 커버 썸네일과 동일 패턴).

검증: `tsc --noEmit`/`eslint`/`next build` 전부 통과. Preview 브라우저로 `/settings`에서 체크박스 토글 시 localStorage(`slidebox:slide-hover-zoom`)가 `"0"`/`"1"`로 정상 반영되는 것 확인, 콘솔 에러 없음. 슬라이드 그리드 자체의 시각적 정렬은 실제 데이터가 있어야 확인 가능해 코드 리뷰까지만 하고 실제 브라우저 확인은 사용자 몫으로 남는다.

### 설정 화면 라이브러리 바로가기 + 커스텀 태그 자동 정리 (2026-07-11)

사용자 제보 2건 처리.

1. **설정 화면에 라이브러리 바로가기 없음** — `AppNav`가 홈(`/home`, 랜딩)/소개/개인정보/설정만 있고 실제 라이브러리 그리드(`/`)로 가는 링크가 없었음. `AppNav`에 "라이브러리"(`href="/"`) 링크 추가. 미연결 상태에서 눌러도 `/`가 알아서 랜딩을 보여주므로 깨지지 않음.
2. **슬라이드에서 주제 태그를 지워도 홈 필터의 주제 목록에 남는 문제** — 원인: `TagEditor`의 "×"(제거)는 그 슬라이드/파일에서 태그 배정만 지우고, 태그 자체 정의(`library.tags`)는 지우지 않는 게 원래 설계였음(설정 화면의 "태그 관리"에서만 사전 자체를 지우도록). 그런데 슬라이드 상세에서 **새로 만든 커스텀 태그**(`is_preset: false`)를 만들고 바로 그 슬라이드에서만 뗀 경우, 아무 데서도 안 쓰이는데 사전에는 계속 남아 사용자 입장에서 "삭제해도 안 없어짐"으로 보임. `src/lib/library-json.ts`에 `pruneUnusedCustomTag(library, tagId)` 추가 — 프리셋이 아니고 파일/슬라이드 어디에도 더 이상 안 쓰이면 사전에서도 함께 제거. `refs/[id]/page.tsx`의 `handleRemoveFileTag`/`handleRemoveSlideTag`에서 태그 배정을 지운 직후 이 함수를 호출하도록 연결. 프리셋 태그(37개)는 사용량과 무관하게 항상 유지됨(대상에서 제외).

검증: `tsc --noEmit`/`eslint`/`next build`/`vitest`(14개) 전부 통과. Preview에서 `/settings`에 "라이브러리" 링크 노출 확인. 태그 자동 정리는 실제 라이브러리 데이터가 있어야 재현 가능해 코드 리뷰까지만 하고 실제 브라우저 확인은 사용자 몫으로 남는다.

### 매번 접속할 때마다 폴더 다시 선택해야 하는 문제 — requestPermission()으로 완화 (2026-07-11)

사용자가 "크롬을 대상으로 하니 이 불편함을 개선할 수 있지 않냐"고 요청. 지금까지는 권한이 없어지면(`needsPermission`) `showDirectoryPicker()`를 다시 호출하는 "폴더 다시 선택" 흐름만 있었다 — 이건 웨일 크래시 우회를 위해 화면 내 재연결을 막고 `/settings`로만 보내던 그 API다.

- **핵심 구분:** `showDirectoryPicker()`(폴더 선택창, 웨일에서 크래시 재현됨)와 `handle.requestPermission()`(이미 저장된 handle에 대해 가벼운 브라우저 권한 프롬프트만 띄움)는 완전히 다른 API다. 지금까지 크래시 우회 조치는 전자에 대한 것이었지, 후자를 막을 이유는 없었다.
- `src/lib/library-dir.ts`에 `requestLibraryPermission(handle)` 추가, `useLibraryDirectory()` 훅이 `requestPermission()` 콜백을 함께 반환하도록 확장(성공 시 `needsPermission`을 자동으로 갱신).
- `page.tsx`/`refs/[id]/page.tsx`/`import/page.tsx`의 권한 필요 화면에 "이 폴더 접근 허용" 버튼 추가(폴더 선택창 없이 권한만 재요청) — 실패하면 안내 문구와 함께 "설정에서 폴더 다시 선택" 링크로 폴백.
- `settings/page.tsx`도 동일하게 "이 폴더 접근 허용"(1차)과 "안 되면 폴더 다시 선택"(폴백)을 함께 노출.
- **주의:** `requestPermission()`은 반드시 버튼 클릭 같은 사용자 제스처 안에서 호출해야 브라우저가 프롬프트를 띄운다(자동 호출 시 무시됨) — 그래서 모든 곳에서 버튼 클릭 핸들러 안에 직접 넣었다.
- 이 기능이 웨일에서도 안전한지는 검증되지 않았다(사용자가 "지금은 크롬 대상"이라고 명시해서 크롬 기준으로 진행). 웨일에서 다시 크래시 리포트가 들어오면 이 버튼이 원인일 가능성도 열어두고 확인할 것.

검증: `tsc --noEmit`/`eslint`/`next build`/`vitest`(19개) 전부 통과. 실제 "권한이 사라진 상태"는 브라우저 재시작 등으로만 재현되고 헤드리스 프리뷰로는 만들 수 없어 코드 리뷰까지만 했다. 사용자가 브라우저를 재시작한 뒤 "이 폴더 접근 허용" 버튼만으로 폴더 재선택 없이 넘어가는지 실제 확인 필요.

### UI 폴리시 2차 — 실제 데이터로 발견한 버그 3건 + 사용법 페이지 신설 (2026-07-11)

전체 검증 마무리 후 사용자가 실제 파일로 몇 가지를 더 확인하다가 진짜 버그를 찾음.

1. **크롬에서 "PDF 열기"가 안 열림.** `open-pdf.ts`의 `openPdfInNewTab`이 파일을 다 읽은 뒤(await 여러 번 지난 뒤) `window.open()`을 호출해서, 클릭이라는 사용자 제스처가 이미 만료된 시점이라 크롬 팝업 차단에 걸림. **수정:** 클릭 즉시(await 이전에) 빈 탭을 먼저 열어 제스처를 소비해두고, 파일을 다 읽은 뒤 그 탭의 `location.href`만 바꾸는 방식으로 변경.
2. `/anthropic-skills:ui-polish`로 재점검 후 발견한 것들: **Geist 폰트가 실제로는 전혀 적용 안 되고 있었음**(`globals.css`의 `body`에 `font-family: Arial, Helvetica, sans-serif`가 하드코딩돼 있어 `layout.tsx`에서 로드한 Geist가 완전히 무시됨 — `var(--font-geist-sans), Arial, Helvetica, sans-serif`로 수정), **다크모드가 절반만 대응**돼 있어 OS 다크모드에서 배경만 검게 바뀌고 카드/글자는 라이트 모드 색 그대로라 깨져 보일 수 있었음(다크모드 지원이 목표가 아니었으므로 `prefers-color-scheme: dark` 미디어쿼리 자체를 제거해 라이트 모드 전용으로 명확히 함), 필터 결과 0개일 때 빈 상태가 라이브러리 텅 빔 상태보다 훨씬 초라했던 것(아이콘 추가), 태그 필터/페이지네이션/카드 등 상호작용 요소에 키보드 포커스 표시가 없었던 것(`focus-visible:ring` 추가), AI 초안 태그(점선 테두리)의 의미가 화면 어디에도 설명되지 있지 않았던 것(상세 화면에 범례 문구 추가).
3. **"기능은 있는데 사용법을 아무도 안 알려준다"는 지적.** 슬라이드 호버 확대 끄기, 미사용 태그 정리, PPTX 전체 경로 등록 같은 기능이 전부 설정 화면에만 있고 다른 곳에서 안내가 전혀 없어서 사용자가 하나씩 찾아야 했음. **전용 `/guide`("사용법") 페이지를 신설**해 시작하기/가져오기/홈 화면/상세 화면/설정 5개 섹션으로 기능을 전부 정리하고, `AppNav`에 "사용법" 링크를 추가했다(라이브러리와 소개 사이).

검증: `tsc --noEmit`/`eslint`/`next build` 전부 통과. `/guide` 페이지 렌더링과 폰트가 실제로 Geist로 바뀌는 것(`getComputedStyle(body).fontFamily`)을 프리뷰 브라우저로 직접 확인했다.

### 로고 재디자인, 푸터 제거, 초기화/내보내기·가져오기 기능 추가 (2026-07-11)

- **로고**: 검정 사각형+"S" 글자가 밋밋하다는 지적으로 겹쳐 쌓인 슬라이드 모양(인디고 3단)으로 새로 디자인(`src/components/Logo.tsx`). AppNav/404/파비콘/OG 이미지까지 통일. **중요 — 배포 전 컨펌 필요:** 시각 디자인을 사용자 확인 없이 바로 배포했다가 지적받음. 이후 색상/로고 같은 시각 디자인 변경은 먼저 시안(visualize 위젯 등)을 보여주고 확정받은 뒤에만 배포하기로 함.
- **푸터 제거**: 랜딩/소개/개인정보/사용법 4개 페이지에만 있던 `MarketingFooter`를 삭제. AppNav가 모든 화면에 이미 동일한 소개/개인정보 링크를 제공해서 중복이었고, "슬라이드박스 — ..." 문구의 대시도 불필요했음.
- **라이브러리 초기화**: `resetLibrary(dirHandle)` 추가 — `thumbs/`를 비우고 `library.json`을 프리셋 상태로 되돌림. 원본 PDF/PPTX는 항상 그대로 둔다는 기존 원칙 유지. 설정에 destructive 스타일(빨간 테두리) 섹션으로 추가, `window.confirm`으로 확인.
- **데이터 내보내기/가져오기**: 사용자가 "컴퓨터를 옮길 때 AI 태깅 비용을 줄이고 싶다"고 제안했는데, 제안한 방식("경로로 인식")은 기술적으로 불가능하다고 정정했다 — File System Access API는 절대경로를 앱에 절대 알려주지 않는다(PPTX 경로 복사 때 이미 확인한 제약과 동일). 대신 이 앱이 이미 파일 식별에 쓰는 **`file_key`(파일명 기준)로 매칭**하는 방식을 택했다.
  - `exportLibraryData`: 태그 사전 + 각 ref의 file_key/제목/메모/tag_ids/ai_tag_ids + 슬라이드별 tag_ids만 JSON으로 내보냄(원본 파일·썸네일 이미지·thumb 경로는 제외 — 새 환경에서는 의미 없는 로컬 경로이기 때문).
  - `importLibraryData`: 현재 라이브러리의 ref와 file_key가 일치하는 것만 병합(아직 안 가져온 파일은 건너뜀). 프리셋 태그는 id가 `kind-슬러그` 형식이라 라이브러리가 달라도 항상 같은 id라 그대로 매칭되지만, 커스텀 태그는 무작위 UUID라 (kind, name)으로 재매칭하고 없으면 새로 만들어 이어붙인다.
  - 사용 흐름: 새 컴퓨터에서 같은 PDF/PPTX들을 먼저 "가져오기"로 라이브러리에 넣은 뒤(썸네일 재생성, AI 태깅은 안 함), 내보내둔 JSON을 "가져오기"하면 파일명이 같은 항목에 태그가 자동 복원되어 AI 재태깅 비용이 안 든다.
  - 단위 테스트 3개(`library-json.test.ts`): 동일 라이브러리 재적용 시 변화 없음, file_key 불일치 시 건너뜀, 커스텀 태그 재매칭.
  - 두 기능 모두 `/guide`(사용법) 페이지의 "설정" 섹션에 설명을 추가했다.

검증: `tsc --noEmit`/`eslint`/`next build`/`vitest`(22개) 전부 통과. 로고는 프리뷰 브라우저로 SVG 렌더링 확인, `/icon`·`/opengraph-image` 200 응답 확인. 초기화/내보내기/가져오기는 실제 폴더 연결이 있어야 끝까지 확인 가능해(헤드리스 프리뷰 한계) 단위 테스트 + 코드 리뷰까지만 했다 — 사용자가 실제로 내보내기→다른 폴더로 가져오기 흐름을 한 번 테스트해봐야 한다.

### 자체 점검 — 내보내기/가져오기 코드에서 결함 4건 발견·수정 (2026-07-11)

사용자가 "같은 폴더에서 내보내기→가져오기했는데 0개 매칭"을 보고한 뒤 자체 점검 요청. `importLibraryData`를 다시 뜯어본 결과:

1. **매칭 0개여도 커스텀 태그가 사전에 쌓이는 부작용(수정).** 기존 코드는 파일 매칭 전에 JSON의 태그 사전 전체를 먼저 병합했다 — 순서 실수로 0개 매칭된 가져오기에서도 커스텀 태그 정의가 현재 라이브러리 사전에 추가됐다. `resolveTagId()`로 바꿔 **매칭된 파일이 실제로 참조하는 태그만** 그때그때 사전에 반영하도록 수정. 매칭 0개면 이제 `library.json` 쓰기 자체를 건너뛴다(진짜 no-op). ⚠️ 사용자의 실패했던 가져오기에서 이 부작용이 이미 발생했을 수 있음 — "미사용 태그 정리" 버튼으로 정리 가능하다고 안내할 것.
2. **삭제된 프리셋 태그가 고아 참조로 붙는 문제(수정).** 프리셋은 존재 확인 없이 id를 그대로 매핑해서, 사용자가 태그 관리에서 프리셋을 지운 상태면 존재하지 않는 태그 id가 파일에 붙었다(UI에서 조용히 무시되는 유령 태그). 이제 사전에 없으면 같은 결정적 id로 프리셋을 되살려서 붙인다.
3. **file_key 비교에 NFC 정규화 추가(방어).** 앱이 만드는 file_key는 이미 NFC지만, macOS를 거친 한글 파일명(NFD)이 섞인 JSON도 안전하게 매칭되도록 비교 시 양쪽 다 `normalize("NFC")`.
4. **에러 메시지가 초록색으로 표시되던 문제(수정).** "올바른 내보내기 파일이 아닙니다" 같은 실패 메시지도 성공과 같은 `text-green-700`이었다. 메시지 상태를 `{text, isError}`로 바꿔 에러는 빨간색으로, 0개 매칭도 에러 취급하고 원인 힌트(1번 순서, 파일명 확인)를 덧붙였다. 표시 시간도 5초→8초.
5. **PDF 열기 실패 시 빈 탭이 남는 문제(수정).** 팝업 차단 우회용으로 미리 연 빈 탭을 파일 읽기 실패 시 `newTab.close()`로 닫고 에러를 다시 던진다.

단위 테스트 24개로 확장(0개 매칭 시 사전 비오염, 삭제된 프리셋 되살리기, NFD↔NFC 매칭 추가). **단, 이 결함들은 사용자가 겪은 "같은 폴더인데 0개 매칭" 자체를 설명하지는 못한다** — 같은 라이브러리에서 내보낸 JSON이면 file_key가 정확히 일치해야 한다. 가장 유력한 설명은 가져오기 시점에 연결돼 있던 라이브러리가 그 5개 ref를 갖고 있지 않았거나(다른 폴더 연결 상태), 이전 테스트 때 받은 다른 JSON을 선택한 경우. 사용자에게 JSON 안 `file_key` 값과 실제 파일명 대조를 요청해둔 상태.

### 네이버 웨일 크래시 마지막 우회 시도 — 드래그 앤 드롭 폴더 연결 (2026-07-11)

이전 세션에서 "웨일 크래시는 앱 코드로 못 고친다, `showDirectoryPicker()` 네이티브 구현 버그"로 결론 내렸었는데, 사용자가 "그래도 고칠 수 있냐"고 다시 물어서 **아직 안 해본 우회 경로 하나**를 시도했다.

- **아이디어:** File System Access API는 폴더 핸들을 얻는 경로가 `showDirectoryPicker()`(선택 창) 말고 하나 더 있다 — `DataTransferItem.getAsFileSystemHandle()`. 사용자가 탐색기에서 폴더를 페이지로 드래그 앤 드롭하면, **선택 창 UI를 전혀 띄우지 않고** 같은 타입의 `FileSystemDirectoryHandle`을 받을 수 있다. 웨일 크래시가 선택 창 UI 쪽 네이티브 버그라면 이 경로는 살아있을 가능성이 있다(다만 더 깊은 핸들/권한 처리 버그라면 이것도 죽을 수 있어 검증 전이라고 사용자에게 명확히 알렸다).
- **구현:**
  - `library-dir.ts`에 `adoptDroppedDirectory(handle)` 추가 — 드롭으로 받은 핸들의 권한을 확인/요청하고(`requestPermission` — drop 이벤트도 사용자 제스처라 프롬프트 가능), `thumbs/` 생성, IndexedDB에 저장. `pickLibraryDirectory()`와 동일한 후처리를 공유하는 별도 함수.
  - `settings/page.tsx`에 점선 테두리 드롭 영역 추가("또는 라이브러리 폴더를 여기로 끌어다 놓아 연결"). `onDrop` 핸들러(`handleFolderDrop`)에서 `e.dataTransfer.items[0].getAsFileSystemHandle()`을 **await 이전에(동기적으로) 먼저 호출**해서 잡아둔다 — dataTransfer 항목은 이벤트 핸들러가 반환되면 무효화되므로, 나중에 접근하면 안 된다는 점에 주의해서 짰다.
  - `/guide`(사용법) 페이지의 "폴더 연결" 항목에도 이 대체 경로를 설명 추가.
- **검증:** `tsc`/`eslint`/`vitest`(24개)/`next build` 전부 통과. 드롭 영역이 화면에 정상 렌더링되는 것은 프리뷰로 확인했지만, **실제 드래그 앤 드롭 동작과 웨일에서 크래시 없이 되는지는 실제 브라우저에서 사용자가 직접 확인해야 한다**(헤드리스 프리뷰로는 파일시스템 드래그 이벤트를 재현할 수 없음).
- **사용자에게 명확히 전달한 기대치:** 이건 "고쳤다"가 아니라 "우회 시도"다. 성공 보장은 없고, 웨일에서 여전히 크래시가 나면 그 경로도 안 되는 것으로 확인되는 것뿐이며, 그때는 정말 방법이 없다(네이버에 직접 제보하는 것 외에는).
- **→ 결과: 드래그 앤 드롭도 웨일에서 실패("역시 웨일은 안되네", 2026-07-11).** `showDirectoryPicker()`(선택 창)와 `getAsFileSystemHandle()`(드래그 앤 드롭) 두 개의 서로 다른 진입 경로가 전부 죽는다는 건, 문제가 선택 창이라는 UI 껍데기가 아니라 **웨일의 File System Access API 구현 자체(핸들 생성/권한 처리 등 더 깊은 공통 경로)**에 있다는 뜻이다. 이제 시도해볼 만한 우회로가 남아있지 않다.
- **최종 결론(확정): 이 앱의 코드로는 웨일 크래시를 고칠 방법이 없다.** 앞으로 이 문제가 다시 언급되면 새로운 우회를 더 찾으려 하지 말 것 — 이미 두 가지 서로 다른 API 경로를 다 시도했고 둘 다 실패했다. 남은 선택지는 (1) 비차단 경고 배너를 계속 유지(현재 상태, 사용자가 하드 차단은 원치 않음), (2) 사용자가 네이버에 직접 크래시를 제보(Crashpad dmp 경로는 앞서 안내함)뿐이다. 드래그 앤 드롭 연결 기능 자체는 유용하니 코드는 그대로 남겨둔다(크롬 등 다른 브라우저에서는 정상 작동하는 대체 연결 수단으로서 가치가 있음).
